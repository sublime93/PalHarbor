...
const defaultDatabasePath = existsSync(resolve(apiDirectory, 'data/paldeck.sqlite'))
...
const configuredUrl = process.env.DATABASE_URL?.trim()
  || pathToFileURL(legacyPath ? resolve(apiDirectory, legacyPath) : defaultDatabasePath).href
const databaseUrl = configuredUrl.startsWith('file:') && !configuredUrl.startsWith('file://')
  ? pathToFileURL(resolve(apiDirectory, configuredUrl.slice('file:'.length))).href
  : configuredUrl
