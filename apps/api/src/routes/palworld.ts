...
import {
  getEndpointRule,
  type PalworldEndpoint,
} from '../palworld/endpoints.js'
import { InvalidGameDataError } from '../palworld/game-data.js'
import {
  InvalidPalworldRequestError,
  validatePalworldRequestBody,
} from '../palworld/input.js'
import {
  PalworldConnectionError,
  PalworldService,
} from '../palworld/service.js'
...
export function createPalworldRoute(
  ...
  return {
    ...
    handler: async (request, reply) => {
      ...
      if (
        request.method === 'POST' &&
        request.headers[REQUEST_MARKER_HEADER] !== '1'
      ) {
        return reply
          .code(403)
      ...
      if (!isGatewayConfigured(config)) {
        return reply.code(503).send({
          error:
            'The Palworld connection is not configured. Set PALWORLD_API_URL, PALWORLD_USERNAME, and PALWORLD_PASSWORD in apps/api/.env, then restart PalHarbor.',
        })
      }

      try {
        const body = validatePalworldRequestBody(
        ...
        if (error instanceof InvalidPalworldRequestError) {
          return reply.code(400).send({ error: error.message })
        }
        const failure =
          error instanceof PalworldConnectionError ? error : undefined
