...
describe('Palworld GameData', () => {
  ...
  it.each([
    ...
    expect(() => normalizeGameDataSnapshot(payload)).toThrow(InvalidGameDataError)
  })
  ...
  it('proxies a text/plain JSON snapshot and preserves Basic auth', async () => {
    ...
    try {
      const response = await app.inject({ method: 'GET', url: '/api/palworld/game-data' })
  ...
  it('turns a malformed successful upstream response into a safe 502', async () => {
    const fetchMock = vi.fn<typeof fetch>(async () => new Response('{"enabled":true}', {
    ...
    try {
      ...
      expect(response.json()).toEqual({
        error: 'The Palworld server returned a malformed world snapshot.',
      })
