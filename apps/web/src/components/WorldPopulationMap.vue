...
import type { ImageOverlay, LatLng, LatLngBounds, Map as LeafletMap, TileLayer, ZoomAnimEvent } from 'leaflet'
import { Grid3X3, Layers3, LocateFixed, MapPinned, Minus, Pickaxe, Plus, Scan, TreePine, X } from '@lucide/vue'
import type { Actor } from '../types'
import {
  CAVE_ENTRANCES,
  CAVE_ENTRANCE_SOURCE,
  actorStage,
  summarizeCaveStages,
  type CaveEntrance,
} from '../cave-map'
...
const terrainUrl = computed(() => regionMeta.value.terrainFile
  ? `${import.meta.env.BASE_URL}maps/${regionMeta.value.terrainFile}`
  : null)
const terrainTileUrl = computed(() => regionMeta.value.terrainTiles
  ? `${import.meta.env.BASE_URL}maps/${regionMeta.value.terrainTiles.pathTemplate}`
  : null)

const caveStageOptions = computed(() => summarizeCaveStages(props.actors.filter((actor) => {
  const position = actorPosition(actor)
  return position && mapRegionForActor(position.x, position.y, actor.Stage) === 'caves'
})))

const positionedActors = computed<PositionedActor[]>(() => props.actors.flatMap((actor) => {
  const position = actorPosition(actor)
  const actorRegion = position ? mapRegionForActor(position.x, position.y, actor.Stage) : null
  const stage = actorStage(actor)
  const matchesCaveStage = region.value !== 'caves'
  ...
  return position && actorRegion === region.value && matchesCaveStage ? [{
    ...
    mapPosition: locationToRegionLeaflet(region.value, position.x, position.y),
  }] : []
}))
const positionedEntrances = computed<PositionedEntrance[]>(() => (
  entrancesVisible.value && region.value !== 'caves'
    ? CAVE_ENTRANCES.flatMap((entrance) => mapRegionForLocation(entrance.x, entrance.y) === region.value
      ? [{ entrance, mapPosition: locationToRegionLeaflet(region.value, entrance.x, entrance.y) }]
      : [])
    : []
))
...
const orderedPositionedActors = computed(() => [...positionedActors.value].sort((left, right) =>
  categoryPriority[left.category] - categoryPriority[right.category],
))
const regionOptions = [MAP_REGIONS.palpagos, MAP_REGIONS['world-tree'], MAP_REGIONS.caves]
const inspectedActor = computed(() => selectedEntrance.value ? null : selectedActor.value ?? hoveredActor.value)
const inspectedEntrance = computed(() => selectedActor.value ? null : selectedEntrance.value ?? hoveredEntrance.value)
const inspectedPosition = computed(() => inspectedActor.value ? actorPosition(inspectedActor.value) : null)
const inspectedCategory = computed<ActorCategory>(() => inspectedActor.value ? actorCategory(inspectedActor.value) : 'Unknown')
const tooltipStyle = computed<CSSProperties>(() => ({
  left: `${tooltipPosition.value.x}px`,
  top: `${tooltipPosition.value.y}px`,
}))
const mapStatus = computed(() => {
  if (terrainState.value === 'error') return 'Terrain image unavailable. Coordinate grid fallback active.'
  ...
  if (!terrainVisible.value) return `Terrain hidden on ${regionMeta.value.label}.`
  if (selectedEntrance.value) return `${selectedEntrance.value.label} selected on the surface map.`
  if (selectedActor.value) return `${actorDisplayName(selectedActor.value)} selected on the map.`
...
const emptyTitle = computed(() => {
  ...
  if (props.actors.length) return `No reported actors in ${regionMeta.value.label}`
  return 'No positioned actors found'
})
const emptyCopy = computed(() => props.loading
...
function gridValues(minimum: number, maximum: number, step: number) {
  const values: number[] = []
  for (let value = Math.ceil(minimum / step) * step; value <= maximum; value += step) values.push(value)
  return values
}
...
function drawCaveSchematic(context: CanvasRenderingContext2D) {
  ...
  const northWest = currentMap.latLngToContainerPoint([FLAT_MAP_BOUNDS.maxY, FLAT_MAP_BOUNDS.minX])
  const southEast = currentMap.latLngToContainerPoint([FLAT_MAP_BOUNDS.minY, FLAT_MAP_BOUNDS.maxX])
  const width = southEast.x - northWest.x
  const height = southEast.y - northWest.y
  const center = currentMap.latLngToContainerPoint([MAP_REGIONS.caves.mapBounds.maxY / 2, MAP_REGIONS.caves.mapBounds.maxX / 2])
  ...
  for (const [ringIndex, radius] of ringRadii.entries()) {
    ...
    for (let index = 0; index <= 32; index += 1) {
      ...
      const wobble = 1 + Math.sin(angle * (3 + ringIndex) + ringIndex * 1.7) * (0.08 + ringIndex * 0.015)
      const mapX = MAP_REGIONS.caves.mapBounds.maxX / 2 + Math.cos(angle) * radius * wobble
      const mapY = MAP_REGIONS.caves.mapBounds.maxY / 2 + Math.sin(angle) * radius * wobble
...
function drawGrid(context: CanvasRenderingContext2D) {
  ...
  const visibleRight = Math.min(viewport?.width ?? Number.POSITIVE_INFINITY, southEast.x)
  const visibleBottom = Math.min(viewport?.height ?? Number.POSITIVE_INFINITY, southEast.y)
  ...
  const stronger = region.value === 'caves' || terrainState.value === 'error' || !terrainVisible.value
  ...
  context.strokeStyle = stronger ? 'rgba(88, 215, 221, .22)' : 'rgba(88, 215, 221, .10)'
  context.beginPath()
  for (const value of gridValues(worldBounds.minX, worldBounds.maxX, minorStep)) {
    if (Math.abs(value / gridStep - Math.round(value / gridStep)) < 0.000_001) continue
    const { y } = locationToRegionMap(region.value, value, centerY)
    const screenY = currentMap.latLngToContainerPoint([y, FLAT_MAP_BOUNDS.minX]).y
    context.moveTo(northWest.x, screenY)
    context.lineTo(southEast.x, screenY)
  }
  for (const value of gridValues(worldBounds.minY, worldBounds.maxY, minorStep)) {
    if (Math.abs(value / gridStep - Math.round(value / gridStep)) < 0.000_001) continue
    const { x } = locationToRegionMap(region.value, centerX, value)
    const screenX = currentMap.latLngToContainerPoint([FLAT_MAP_BOUNDS.minY, x]).x
    context.moveTo(screenX, northWest.y)
    context.lineTo(screenX, southEast.y)
  }
  context.stroke()

  context.strokeStyle = stronger ? 'rgba(88, 215, 221, .50)' : 'rgba(88, 215, 221, .25)'
  context.beginPath()
  for (const value of gridValues(worldBounds.minX, worldBounds.maxX, gridStep)) {
    const { y } = locationToRegionMap(region.value, value, centerY)
    const screenY = currentMap.latLngToContainerPoint([y, FLAT_MAP_BOUNDS.minX]).y
    context.moveTo(northWest.x, screenY)
    context.lineTo(southEast.x, screenY)
  }
  for (const value of gridValues(worldBounds.minY, worldBounds.maxY, gridStep)) {
    const { x } = locationToRegionMap(region.value, centerX, value)
    const screenX = currentMap.latLngToContainerPoint([FLAT_MAP_BOUNDS.minY, x]).x
    context.moveTo(screenX, northWest.y)
    context.lineTo(screenX, southEast.y)
  ...
  context.stroke()
  context.strokeStyle = stronger ? 'rgba(88, 215, 221, .64)' : 'rgba(88, 215, 221, .35)'
  ...
  context.fillStyle = stronger ? 'rgba(190, 245, 244, .9)' : 'rgba(190, 235, 235, .72)'
  for (const value of gridValues(worldBounds.minX, worldBounds.maxX, gridStep)) {
    const { y } = locationToRegionMap(region.value, value, centerY)
    const screenY = currentMap.latLngToContainerPoint([y, FLAT_MAP_BOUNDS.minX]).y
...
function clampTooltip(x: number, y: number) {
  ...
  if (Math.abs(next.x - tooltipPosition.value.x) > 0.5
...
function draw() {
  ...
  entranceScreenPoints = positionedEntrances.value.map((item) => {
    const point = currentMap.latLngToContainerPoint([item.mapPosition.lat, item.mapPosition.lng])
  ...
  for (const point of entranceScreenPoints) {
    if (point.screenX < -20 || point.screenX > rect.width + 20
      || point.screenY < -20 || point.screenY > rect.height + 20) continue
  ...
  const pointsByCategory = Object.fromEntries(
    (Object.keys(actorCategoryMeta) as ActorCategory[]).map((category) => [category, [] as ScreenPoint[]]),
  ...
  for (const category of Object.keys(actorCategoryMeta) as ActorCategory[]) {
    ...
    context.globalAlpha = category === 'WildPal' || category === 'NPC' ? 0.72 : 0.94
  ...
  if (selectedActor.value) {
    const selectedPoint = screenPoints.find(({ actor }) => actor === selectedActor.value)
    if (selectedPoint) {
      ...
      context.beginPath()
      context.arc(selectedPoint.screenX, selectedPoint.screenY, markerRadius(selectedPoint.category) + 5, 0, Math.PI * 2)
      context.stroke()
      context.shadowBlur = 0
  ...
  if (selectedEntrance.value) {
    const selectedPoint = entranceScreenPoints.find(({ entrance }) => entrance === selectedEntrance.value)
    if (selectedPoint) {
      ...
      context.shadowBlur = 7
      context.beginPath()
      context.arc(selectedPoint.screenX, selectedPoint.screenY, 10, 0, Math.PI * 2)
      context.stroke()
      context.shadowBlur = 0
      clampTooltip(selectedPoint.screenX, selectedPoint.screenY)
    }
...
function nearestTarget(event: MouseEvent | PointerEvent): MapHitTarget | null {
  ...
  for (let index = interactiveEntranceScreenPoints.length - 1; index >= 0; index -= 1) {
...
function inspectPoint(event: PointerEvent) {
  ...
  if (pointerStart) {
    pointerTravel = Math.max(pointerTravel, Math.hypot(
  ...
  if (event.pointerType === 'mouse' && !pointerStart) {
    ...
    hoveredEntrance.value = target?.kind === 'entrance' ? target.point.entrance : null
  }
}
...
function selectPoint(event: MouseEvent) {
  ...
  selectedEntrance.value = target?.kind === 'entrance' ? target.point.entrance : null
  requestDraw()
}
...
function fitMap() {
  if (map && mapBounds) map.fitBounds(mapBounds, { animate: true, padding: [12, 12] })
}
...
function loadRegionTerrain() {
  ...
  interactiveEntranceScreenPoints = []
  replaceTerrainLayer?.()
  if (map && mapBounds) map.fitBounds(mapBounds, { animate: false, padding: [12, 12] })
  updateZoomLabel()
  requestDraw()
}

function projectedContainerPoint(lat: number, lng: number, event: ZoomAnimEvent) {
  if (!map) return null
  return map.project([lat, lng], event.zoom)
...
function animateCanvasZoom(event: ZoomAnimEvent) {
  ...
  const targetTopLeft = projectedContainerPoint(baseTopLeft.lat, baseTopLeft.lng, event)
  ...
  if (selectedActor.value) {
    const selected = positionedActors.value.find(({ actor }) => actor === selectedActor.value)
    if (selected) {
      const target = projectedContainerPoint(selected.mapPosition.lat, selected.mapPosition.lng, event)
      if (target) clampTooltip(target.x, target.y)
    }
  } else if (selectedEntrance.value) {
    const selected = positionedEntrances.value.find(({ entrance }) => entrance === selectedEntrance.value)
    if (selected) {
      const target = projectedContainerPoint(selected.mapPosition.lat, selected.mapPosition.lng, event)
      if (target) clampTooltip(target.x, target.y)
    }
...
async function initializeMap() {
  ...
  map.attributionControl.addAttribution('Map © Pocketpair, Inc. · Unofficial fan project')
  map.attributionControl.addAttribution(`Cave entrances: <a href="${CAVE_ENTRANCE_SOURCE.url}" target="_blank" rel="noreferrer">1.0 map data</a>`)
  ...
  replaceTerrainLayer = () => {
    ...
    previousLayer?.off()
    const tileSet = regionMeta.value.terrainTiles
    ...
    const loadFallbackImage = () => {
      ...
      const fallbackLayer = L.imageOverlay(
        fallbackUrl,
        mapBounds!,
        { alt: `${regionMeta.value.label} terrain`, opacity: terrainVisible.value ? 0.9 : 0 },
      )
    ...
    nextLayer.on('tileload', () => { loadedTileCount += 1 })
    nextLayer.on('tileerror', () => { failedTileCount += 1 })
    nextLayer.on('load', () => {
...
onMounted(() => {
  ...
  void nextTick(() => { void initializeMap() })
})
...
watch(caveStageOptions, (options) => {
  ...
  } else if (activeCaveStage.value !== 'all' && !options.some(({ stage }) => stage === activeCaveStage.value)) {
    activeCaveStage.value = options[0]?.stage ?? 'all'
  }
  requestDraw()
})
...
<template>
  <div
    ...
    @pointerleave="hoveredActor = null; hoveredEntrance = null; pointerStart = undefined"
    @click="selectPoint"
  >
    <div
      ref="mapElement"
    ...
    <div class="world-map-regions" role="group" aria-label="World map region">
      ...
      >
        ...
        {{ candidate.id === 'palpagos' ? 'Islands' : candidate.id === 'world-tree' ? 'World Tree' : 'Caves' }}
      </button>
    </div>
    ...
    >
      ...
      <select :value="activeCaveStage" :disabled="!caveStageOptions.length" @change="selectCaveStage">
        <option v-if="!caveStageOptions.length" value="all">Waiting for an interior Stage</option>
        <option v-else value="all">All live stages · {{ caveStageOptions.reduce((sum, item) => sum + item.count, 0) }} actors</option>
        <option v-for="item in caveStageOptions" :key="item.stage" :value="item.stage" :title="item.stage">
    ...
    <div class="world-map-controls" aria-label="Map controls">
      <button type="button" title="Zoom in" aria-label="Zoom in" @click.stop="zoomIn"><Plus :size="16" /></button>
      <button type="button" title="Zoom out" aria-label="Zoom out" @click.stop="zoomOut"><Minus :size="16" /></button>
      <button type="button" title="Fit the full map" aria-label="Fit the full map" @click.stop="fitMap"><Scan :size="15" /></button>
      <button
        v-if="region !== 'caves'"
        type="button"
      ...
      ><Pickaxe :size="15" /></button>
      <span>{{ zoomLabel }}</span>
    </div>

    <div v-if="terrainState === 'error' && region !== 'caves'" class="world-map-fallback" role="status">
      {{ regionMeta.label }} terrain unavailable · grid fallback active
    </div>

    <div v-if="!positionedActors.length" class="world-map-empty" :class="{ 'cave-empty': region === 'caves' }">
    ...
    <div v-if="inspectedActor && inspectedPosition" class="map-inspector" :class="{ pinned: selectedActor }" :style="tooltipStyle">
      <button v-if="selectedActor" type="button" aria-label="Close actor details" @click.stop="clearSelection"><X :size="13" /></button>
      <div class="map-inspector-title">
        <span :style="{ backgroundColor: actorCategoryMeta[inspectedCategory].color }" />
        <strong>{{ actorDisplayName(inspectedActor) }}</strong>
      </div>
      <p>{{ actorCategoryMeta[inspectedCategory].label }}<template v-if="inspectedActor.level != null"> · Level {{ inspectedActor.level }}</template></p>
      <dl>
        <div><dt>X</dt><dd>{{ Math.round(inspectedPosition.x).toLocaleString() }}</dd></div>
        <div><dt>Y</dt><dd>{{ Math.round(inspectedPosition.y).toLocaleString() }}</dd></div>
        <div><dt>Z</dt><dd>{{ inspectedPosition.z == null ? '—' : Math.round(inspectedPosition.z).toLocaleString() }}</dd></div>
      </dl>
      <div v-if="actorStage(inspectedActor)" class="map-inspector-stage"><span>Stage</span><code>{{ actorStage(inspectedActor) }}</code></div>
    </div>

    <div v-else-if="inspectedEntrance" class="map-inspector entrance-inspector" :class="{ pinned: selectedEntrance }" :style="tooltipStyle">
      <button v-if="selectedEntrance" type="button" aria-label="Close cave entrance details" @click.stop="clearSelection"><X :size="13" /></button>
      <div class="map-inspector-title">
      ...
      <dl>
        <div><dt>X</dt><dd>{{ Math.round(inspectedEntrance.x).toLocaleString() }}</dd></div>
        <div><dt>Y</dt><dd>{{ Math.round(inspectedEntrance.y).toLocaleString() }}</dd></div>
        <div><dt>LAYER</dt><dd>ENTRANCE</dd></div>
      </dl>
    </div>

    <div class="map-coordinate-note">
      <LocateFixed :size="12" />
      {{ region === 'caves' ? 'Local −10k…+10k instance plane · schematic, not terrain' : 'Live X/Y grid · gold diamonds are cave entrances' }}
    </div>
