...
describe('Palworld client', () => {
  ...
  it('reads documented endpoints through the local gateway', async () => {
    const fetchMock = vi.fn(async () => new Response(JSON.stringify({ serverfps: 60 }), {
      status: 200,
    ...
    expect(fetchMock).toHaveBeenCalledWith('/api/palworld/metrics', expect.objectContaining({ method: 'GET' }))
  })
  ...
  it('loads world actor snapshots through the GameData gateway route', async () => {
    const snapshot = {
      ...
      FPS: 59.8,
      AverageFPS: 58.9,
      ActorData: [{ Type: 'PalBox', Name: 'Main Base', LocationX: 1200, LocationY: -800 }],
    }
    ...
    expect(fetchMock).toHaveBeenCalledWith('/api/palworld/game-data', expect.objectContaining({ method: 'GET' }))
  })
  ...
  it('sends action bodies as JSON', async () => {
    ...
    await palworldApi('announce', { method: 'POST', body: { message: 'Hello Pals' } })
    expect(fetchMock).toHaveBeenCalledWith('/api/palworld/announce', expect.objectContaining({
      method: 'POST',
  ...
  it('marks bodyless mutations as Paldeck requests', async () => {
    const fetchMock = vi.fn(async () => new Response(JSON.stringify({ ok: true }), { status: 200 }))
    vi.stubGlobal('fetch', fetchMock)

    await palworldApi('save', { method: 'POST' })
    expect(fetchMock).toHaveBeenCalledWith('/api/palworld/save', expect.objectContaining({
      method: 'POST',
  ...
  it('surfaces gateway error messages', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => new Response(JSON.stringify({ error: 'Server unavailable' }), { status: 502 })))
  ...
  it('loads an activity summary for the selected reporting period', async () => {
    ...
    expect(result.topPlayers[0]).toMatchObject({ averageLatencyMs: 47, latencySampleCount: 125 })
    expect(result.ipHistory[0]?.ipAddress).toBe('172.16.0.42')
    expect(fetchMock).toHaveBeenCalledWith('/api/activity/summary?days=30', { signal: undefined })
  })

  it('surfaces activity API errors consistently', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => new Response(JSON.stringify({ error: 'Activity store unavailable' }), { status: 503 })))
