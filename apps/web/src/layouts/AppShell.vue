  ...
  <div class="app-shell">
    <aside class="sidebar">
      <div class="brand-lockup">
        ...
        <div>
          <div class="brand-name">PALHARBOR</div>
      ...
      <div class="server-mini-card">
        ...
        <div class="mini-server-name">
          {{ info?.servername || '172.16.0.171' }}
        </div>
        <div class="mini-meta">
          {{ playerCount }} / {{ maxPlayers || '—' }} players
        </div>
        <div class="capacity-track">
          <span :style="{ width: `${playerCapacity}%` }" />
        </div>
      </div>
    ...
    <main class="workspace">
        ...
        <div class="topbar-actions">
          <div class="refresh-control">
            ...
            <select
              ...
              @change="
                setRefresh(Number(($event.target as HTMLSelectElement).value))
              "
            >
      ...
      <div
        v-if="dominantError && !connected"
        class="notice-banner error-banner"
        role="alert"
      >
        <CircleAlert :size="18" />
        <div>
          <strong>Couldn’t reach the server.</strong
          ><span>{{ dominantError }}</span>
        </div>
        <button class="text-button" @click="refreshCore(true)">
          Try again
        </button>
      </div>
    ...
    <div
      ...
      @click.self="!actionBusy && (confirmation = null)"
    >
      <div
      ...
      >
        ...
        <div class="confirm-icon" :class="{ danger: confirmation.danger }">
          <CircleAlert v-if="confirmation.danger" :size="23" /><ServerCog
            v-else
            :size="23"
          />
        </div>
        ...
        <label v-if="confirmation.phrase" class="stacked-field confirm-phrase"
          ><span
            >Type <strong>{{ confirmation.phrase }}</strong> to continue</span
          ><input v-model="confirmationInput" autofocus autocomplete="off"
        /></label>
        <div class="modal-actions">
          ...
          >
            Cancel</button
          ><button
            :class="confirmation.danger ? 'danger-button' : 'primary-button'"
            :disabled="
              actionBusy ||
              Boolean(
                confirmation.phrase &&
                confirmationInput !== confirmation.phrase,
              )
            "
            @click="confirmAction"
          >
            <RefreshCw v-if="actionBusy" :size="15" class="spin-once" />{{
              confirmation.label
            }}
          </button>
        </div>
