...
const maxDailyPlaytime = computed(() =>
  Math.max(
    1,
...
const chartWidth = computed(
  () => `${Math.max(620, (activitySummary.value?.daily.length ?? 0) * 27)}px`,
)
  ...
  <section class="page-content activity-page">
    <div class="page-intro activity-intro">
      <div>
        ...
        <p>
          Connection history, playtime, and connection quality recorded by
          PalHarbor.
        </p>
      </div>
      <div class="activity-controls">
        ...
        <select
          ...
          :value="activityDays"
          @change="
            setActivityDays(Number(($event.target as HTMLSelectElement).value))
          "
        >
    ...
    >
      <span class="collector-state"
        ><i />Collector {{ activitySummary.collector.status }}</span
      >
      <span
        >Last successful sample:
        {{
          collectorTime(activitySummary.collector.lastSuccessfulPollAt)
        }}</span
      >
      <span
        >Sampling every
        {{ activitySummary.collector.pollIntervalSeconds }} seconds</span
      >
      <span
        >Retained for {{ activitySummary.collector.retentionDays }} days</span
      >
    </div>
    ...
    <div v-if="errors.activity" class="inline-error" role="alert">
      ...
      <button class="text-button" @click="refresh('activity')">
        Try again
      </button>
    </div>

    <div
      v-if="activityLoading && !activitySummary"
      class="activity-stat-grid"
      aria-label="Loading activity statistics"
    >
      <div v-for="n in 5" :key="n" class="activity-stat-card skeleton-card">
        <span /><span /><span />
      </div>
    </div>
    <div v-else class="activity-stat-grid">
      <article class="activity-stat-card accent-cyan">
        ...
        <small
          >in the last
          {{ activitySummary?.periodDays ?? activityDays }} days</small
        >
      </article>
      ...
      <article class="activity-stat-card accent-violet">
        ...
        <strong class="compact">{{
          formatDuration(totals?.totalPlaytimeSeconds)
        }}</strong>
      ...
      <article class="activity-stat-card accent-green">
        ...
        <strong class="compact">{{
          formatDuration(totals?.averageSessionSeconds)
        }}</strong>
    ...
    <div class="activity-main-grid">
      <article class="panel activity-chart-panel">
        <div class="panel-header">
          <div>
            <span class="panel-kicker">DAILY PLAYTIME</span>
          ...
          <div class="activity-chart" :style="{ width: chartWidth }">
            ...
            >
              <div class="activity-bar-track">
                <span
                  :style="{
                    height: `${Math.max(day.playtimeSeconds ? 4 : 0, (day.playtimeSeconds / maxDailyPlaytime) * 100)}%`,
                  }"
                />
              </div>
              <small
                :class="{
                  hidden: !dayLabelVisible(index, activitySummary.daily.length),
                }"
                >{{ formatDay(day.date) }}</small
              >
            </div>
          </div>
        </div>
        <div v-else class="empty-state activity-empty">
          <Activity :size="30" /><strong>No activity yet</strong
          ><span
            >Daily playtime will appear after player sessions are
            recorded.</span
          >
        </div>
      </article>
      ...
      <article class="panel leaderboard-panel">
        <div class="panel-header">
          <div>
            <span class="panel-kicker">TOP PLAYERS</span>
        ...
        <div v-if="activitySummary?.topPlayers.length" class="leaderboard-list">
          <div
            v-for="(player, index) in activitySummary.topPlayers.slice(0, 8)"
            :key="player.userId"
            class="leaderboard-row"
          >
            <span class="leaderboard-rank" :class="{ podium: index < 3 }">{{
              index + 1
            }}</span>
            <div class="avatar">
            ...
            <div class="leaderboard-player">
              <strong
                ...
                <i v-if="player.online" class="online-pip"
              /></strong>
              <span :title="player.userId"
                >{{ shortId(player.userId) }} ·
                {{ player.sessionCount }} sessions</span
              >
            </div>
            <div class="leaderboard-stats">
              <strong class="leaderboard-time">{{
              ...
              >
                <Activity :size="9" />
                {{ formatNumber(player.averageLatencyMs) }} ms ·
                {{ latencyQuality(player.averageLatencyMs).label }}
              </span>
              <span
                v-else
                class="latency-quality unknown"
                title="No valid latency samples in this reporting period."
              >
        ...
        <div v-else class="empty-state activity-empty">
          <Trophy :size="30" /><strong>No leaderboard yet</strong
          ><span>Player totals will be ranked here.</span>
        </div>
      </article>
    </div>
    ...
    <article class="ip-history-section">
      <div class="panel-header recent-header">
        <div>
          <span class="panel-kicker">CONNECTION ADDRESSES</span>
          <h3>Player IP history</h3>
        </div>
        <span class="ip-history-note"
          ><Network :size="13" /> Last observed in this period · counts reflect
          retained observations</span
        >
      </div>
      <div class="table-card ip-history-table-card">
        <table
          v-if="activitySummary?.ipHistory?.length"
          class="data-table ip-history-table"
        >
          <thead>
            <tr>
              <th>Player</th>
          ...
          <tbody>
            <tr
              v-for="entry in activitySummary?.ipHistory ?? []"
              :key="`${entry.userId}:${entry.ipAddress}`"
            >
              <td>
                <div class="player-cell">
                  ...
                  <div>
                    <strong>{{ entry.playerName || 'Unknown player' }}</strong
              ...
              <td>
                <span
                  ...
                  :class="entry.online ? 'online' : 'complete'"
                  ><i />{{ entry.online ? 'Online' : 'Offline' }}</span
                >
              </td>
            </tr>
          </tbody>
        </table>
        <div v-else class="empty-state ip-history-empty">
          <Network :size="34" />
          <strong>{{
            activitySummary?.collector.storesIpAddresses
              ? 'No IP history recorded'
              : 'IP storage is disabled'
          }}</strong>
          <span v-if="activitySummary?.collector.storesIpAddresses"
            >Addresses will appear here after players connect during this
            reporting period.</span
          >
          <span v-else
            >PalHarbor does not store player connection addresses unless
            STORE_PLAYER_IPS=true.</span
          >
        </div>
      </div>
    </article>
    ...
    <article class="recent-activity-section">
      <div class="panel-header recent-header">
        <div>
          <span class="panel-kicker">CONNECTION LOG</span>
          <h3>Recent sessions</h3>
        </div>
        <span v-if="activitySummary?.generatedAt" class="activity-generated"
          >Updated {{ formatDateTime(activitySummary.generatedAt) }}</span
        >
      </div>
      <div class="table-card activity-table-card">
        <table
          v-if="activitySummary?.recentSessions.length"
          class="data-table activity-table"
        >
          <thead>
            <tr>
              <th>Player</th>
          ...
          <tbody>
            <tr
              v-for="session in activitySummary.recentSessions"
              :key="session.id"
            >
              <td>
                <div class="player-cell">
                  <div class="avatar">
                    {{ (session.playerName || '?').slice(0, 1).toUpperCase() }}
                  </div>
                  <div>
                    <strong>{{ session.playerName || 'Unknown player' }}</strong
                    ><span :title="session.userId">{{
              ...
              <td>
                <span
                  class="session-status"
                  :class="session.online ? 'online' : 'complete'"
                  ><i />{{ session.online ? 'Online' : 'Complete' }}</span
                >
              </td>
            </tr>
          </tbody>
        ...
        <div v-else class="empty-state">
          <History :size="34" /><strong>No sessions recorded</strong
          ><span
            >Connections will be logged automatically when players join.</span
          >
        </div>
      </div>
    </article>
